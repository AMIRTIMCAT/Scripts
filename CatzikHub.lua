-- Подключаем библиотеку redz-V5-remake с проверками
local Library
do
    ...
    if not success or type(lib) ~= "table" then
        warn("Ошибка загрузки библиотеки redz-V5-remake")
        return
    end
    Library = lib
end

-- Проверяем, есть ли MakeWindow
if type(Library.MakeWindow) ~= "function" then
    warn("Метод MakeWindow не найден в библиотеке")
    return
end
...
if not Window or type(Window.Page) ~= "function" then
    warn("Объект Window не содержит метод Page")
    return
end
...
-- Обновляем список зон для телепортации
...
-- Обновляем опции в dropdown каждые 10 секунд
spawn(function()
    while true do
        task.wait(10)
        updateOptions()
