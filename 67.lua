...
local function stealAndReturn()
	...
	if prompt then
		local success, err = pcall(function()
			-- Проверяем доступность функции fireproximityprompt
			...
			if hasFirePrompt then
				...
				-- Альтернативный способ активации промпта
				if prompt:IsA("ProximityPrompt") then
					...
					error("Объект не является ProximityPrompt")
				end
			end
		end)
		...
		if success then
			...
			warn("Ошибка активации промпта:", err)
		end
	else
