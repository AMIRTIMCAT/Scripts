...
-- ФУНКЦИОНАЛ
-- Добавь этот код после кнопки Reset (примерно в строку 250)

-- Кнопка Instant Steal
local stealButton = Instance.new("TextButton")
stealButton.Name = "StealButton"
stealButton.Size = UDim2.new(1, 0, 0, 35)
stealButton.Position = UDim2.new(0, 0, 0, 205)
stealButton.BackgroundColor3 = Color3.fromRGB(220, 20, 60)
...
local basePosition = Vector3.new(0, 10, 0) -- Позиция базы (настрой под свою игру)
local stealCooldown = false

-- Функция Instant Steal
local function instantSteal()
    ...
    local currentPosition = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    ...
    -- Телепорт к ближайшему игроку (симуляция кражи)
    local closestPlayer = nil
    local closestDistance = 50 -- Максимальная дистанция
    
    for _, otherPlayer in pairs(Players:GetPlayers()) do
        if otherPlayer ~= player and otherPlayer.Character then
            local humanoidRootPart = otherPlayer.Character:FindFirstChild("HumanoidRootPart")
            if humanoidRootPart then
                local distance = (currentPosition.Position - humanoidRootPart.Position).Magnitude
    ...
    if closestPlayer then
        -- Телепорт к цели
        currentPosition.CFrame = closestPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0, 0, -3)
        
        -- Симуляция "кражи" - можно добавить эффекты
        print("🎯 Украл у " .. closestPlayer.Name)
        
        -- Автоматический возврат на базу через 0.5 секунд
        task.wait(0.5)
        
        -- Возврат на базу
        currentPosition.CFrame = CFrame.new(basePosition)
        stealButton.Text = "🏠 RETURNED TO BASE"
        
        -- Эффект успешного возврата
...
-- Обработчик кнопки Instant Steal
stealButton.MouseButton1Click:Connect(function()
    if not stealCooldown then
        instantSteal()
    end
end)

-- Добавь также в адаптацию для мобильных устройств (примерно строка 380)
if UserInputService.TouchEnabled then
    -- ... существующий код ...
    stealButton.Size = UDim2.new(1, 0, 0, 45)
    stealButton.Position = UDim2.new(0, 0, 0, 225)
end

-- Добавь в анимации при наведении
setupButtonHover(stealButton)

-- И увеличь высоту главного окна для новой кнопки (в начале скрипта)
mainFrame.Size = UDim2.new(0, 300, 0, 320) -- Было 280, стало 320
